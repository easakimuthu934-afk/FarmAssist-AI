  ...
  return `
    <div class="auth">
      <div class="auth-card">
        ...
        <button class="demo-button" onclick="loadDemo()">▶ Load demo farm</button>
...
function aiDecisionSummary() {
  const insights = [];
  const costs = data.expenses.reduce((sum, x) => sum + Number(x.amount || 0), 0);
  const revenue = data.harvests.reduce((sum, x) => sum + Number(x.quantity || 0) * Number(x.price || 0), 0);
  const recent = [...data.reports].sort((a,b) => new Date(b.date)-new Date(a.date)).slice(0, 5);

  if (!data.crops.length) {
    return ['Add a crop to activate FarmAssist\'s stage-aware decision engine.'];
  }

  data.crops.forEach(crop => {
    const stage = cropStage(crop);
    insights.push(`${cropName(crop.crop)} is in ${stageName(stage.stage)} (day ${stage.days}). Prioritize crop-condition and soil-moisture checks today.`);
  });

  const attention = recent.filter(r => r.conditionKey === 'needsAttention').length;
  if (attention) insights.push(`${attention} recent report${attention > 1 ? 's' : ''} marked the crop as needing attention. Review those observations before the next field visit.`);

  const pest = recent.filter(r => r.pestObservation).length;
  const disease = recent.filter(r => r.diseaseObservation).length;
  if (pest || disease) insights.push(`Recent records contain ${pest} pest and ${disease} disease observation${(pest+disease) === 1 ? '' : 's'}. Keep these items on the next inspection checklist.`);

  if (data.weather?.risk) insights.push(`Weather signal: elevated rain risk detected. Check drainage and avoid unnecessary irrigation until conditions are clearer.`);

  if (costs > 0) {
    const margin = revenue - costs;
    insights.push(`Financial signal: ₹${costs.toLocaleString('en-IN')} recorded as costs and ₹${revenue.toLocaleString('en-IN')} as harvest revenue, giving a current recorded balance of ₹${margin.toLocaleString('en-IN')}.`);
  }
...
function assistant() {
  const insights = aiDecisionSummary();
  return `
    <div class="section-head">
      <div>
        <span class="tag">🧠 AI Decision Copilot</span>
        <h1>FarmAssist AI</h1>
        <p class="muted">Context-aware recommendations generated from crop stage, farm history, weather signals and financial records.</p>
      </div>
    </div>

    <div class="grid2">
      <section class="card">
        <div class="section-head"><div><h2>What should I do today?</h2><p class="muted">The engine ranks the most relevant actions from your saved records.</p></div></div>
        <div id="answer" class="ai-answer">${insights.map((x,i)=>`<div class="insight"><span>${i+1}</span><div>${esc(x)}</div></div>`).join('')}</div>
      </section>

      <section class="card">
        <h3>Ask FarmAssist</h3>
        <div class="actions" style="margin:14px 0">
          <button class="secondary" onclick="ask('today')">🌱 Today</button>
          <button class="secondary" onclick="ask('cost')">💰 Costs</button>
          <button class="secondary" onclick="ask('history')">📒 History</button>
        </div>
        <div class="small muted">AI layer: deterministic, explainable decision logic for the MVP. It never invents field observations.</div>
      </section>
    </div>
  `;
}
...
function ask(type) {
  ...
  if (type === 'cost') {
    ...
    answer = `Recorded farm expenses total ₹${total.toLocaleString('en-IN')}.`;
  } else if (type === 'history') {
    answer = `Farm history contains ${data.farms.length} farm(s), ${data.crops.length} crop(s), ${data.reports.length} daily report(s), ${data.expenses.length} expense(s), and ${data.harvests.length} harvest record(s).`;
  } else {
    answer = aiDecisionSummary().join(' ');
  }
  const el = document.getElementById('answer');
  if (el) el.innerHTML = `<div class="insight"><span>✦</span><div>${esc(answer)}</div></div>`;
}

function loadDemo() {
  const farmId = id();
  const cropId = id();
  const planted = new Date(Date.now() - 42 * 86400000).toISOString().slice(0,10);
  data.user = {id:id(), name:'Demo Farmer', email:'demo@farmassist.app', language:'en', location:'Madurai', experience:'8', createdAt:new Date().toISOString()};
  data.language = 'en';
  data.loggedIn = true;
  data.farms = [{id:farmId,name:'Green Valley Farm',location:'Madurai',area:'4.5',createdAt:new Date().toISOString()}];
  data.crops = [{id:cropId,crop:'tomato',farmId,farmName:'Green Valley Farm',planted}];
  data.reports = [{id:id(),date:new Date().toISOString(),cropId,work:'Completed drip irrigation and field inspection',irrigation:true,conditionKey:'average',condition:'average',pestObservation:'Few leaf miner marks observed',diseaseObservation:'',labour:3,cost:450,note:'Monitor affected plants'}];
  data.expenses = [{id:id(),date:new Date().toISOString(),category:'Inputs',amount:3200},{id:id(),date:new Date().toISOString(),category:'Labour',amount:1800},{id:id(),date:new Date().toISOString(),category:'Irrigation',amount:900}];
  data.harvests = [{id:id(),date:new Date().toISOString(),quantity:85,unit:'kg',price:38}];
  data.activities = [];
  data.weather = null;
  page = 'home';
  save();
